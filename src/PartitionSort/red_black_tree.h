...
public:
  ...
  int exactQueryPriority(const Packet& q, size_t level, FieldOrder_t fieldOrder, int priority_so_far);
  bool insertHelp(RedBlackTree_node* z, const std::vector<Range1d>& b, size_t level,
		  FieldOrder_t fieldOrder, int priority,  RedBlackTree_node*& out_ptr);
  ...
  int exactQueryIterative(const Packet& q, FieldOrder_t fieldOrder, size_t level=0);

  int calculateMemoryConsumptionRecursion(RedBlackTree_node * node, size_t level, FieldOrder_t fieldOrder);
